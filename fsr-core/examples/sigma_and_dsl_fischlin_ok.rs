//! DSL-based Sigma AND using Fischlin with coverage requirements.
...
fn main() -> Result<()> {
    let g = G1(11);
    let w0 = Scalar(111_111);
    let w1 = Scalar(222_222);
    let y0 = g.pow(w0);
    let y1 = g.pow(w1);
    let sid = b"sigma-and-dsl-fischlin-ok";
    let mut rng = StdRng::seed_from_u64(7070);

    let statement = {
        let mut v = Vec::new(); v.extend_from_slice(&enc_u64(g.0)); v.extend_from_slice(&enc_u64(y0.0)); v.extend_from_slice(&enc_u64(y1.0)); v
    };
    let rho: u16 = 32; let b_bits: u8 = 8;

    let proof = fsr_proof_dsl::prove! {
        transform = "fischlin",
        oracle = FischlinOracle::new(HashOracle::new(b"YavOracle/SigmaAND/DSL/Fischlin"), FischlinParams::new(rho, b_bits)),
        rho = rho, b = b_bits,
        ...
        respond = |_i: usize, e_bytes: &[u8], sigma: &(u64, u64)| {
            ...
            let (r0u, r1u) = *sigma;
            let r0 = Scalar(r0u % ORDER_Q); let r1 = Scalar(r1u % ORDER_Q);
    ...
    let ok = fsr_proof_dsl::verify! {
        transform = "fischlin",
        oracle = FischlinOracle::new(HashOracle::new(b"YavOracle/SigmaAND/DSL/Fischlin"), FischlinParams::new(rho, b_bits)),
        statement = &statement, sid = sid, proof = &proof,
        require = ["c_0", "c_1"],
        bind = |o: &mut _, _i: usize, m_bytes: &[u8]| {
            let t0b = &m_bytes[0..8]; let t1b = &m_bytes[8..16];
            fsr_core::TranscriptRuntime::absorb(o, "c_0", t0b);
        ...
        sigma_verify = |_i: usize, m_bytes: &[u8], e_bytes: &[u8], z_bytes: &[u8]| {
            let t0 = G1(dec_le_u64(&m_bytes[0..8])); let t1 = G1(dec_le_u64(&m_bytes[8..16]));
            let e = Scalar(dec_le_u64(e_bytes) % ORDER_Q);
    ...
    println!("Sigma-AND DSL/Fischlin verify = {}", ok);
