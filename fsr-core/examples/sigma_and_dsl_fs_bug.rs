//! DSL-based Sigma AND with missing c_0 bind; should fail at compile time.
...
fn main() -> Result<()> {
    ...
    let y0 = g.pow(w0);
    let y1 = g.pow(w1);
    let sid = b"sigma-and-dsl-fs-bug";
    ...
    let _ = fsr_proof_dsl::prove! {
        transform = "fs",
        oracle = HashOracle::new(b"YavOracle/SigmaAND/DSL/FS/Bug"),
        rho = 1u16,
        b = 0u8,
