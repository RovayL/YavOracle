//! DSL-based Sigma OR using Fischlin with a coverage bug (missing c_0); compile-time failure.
...
fn main() -> Result<()> {
    let g = G1(5);
    let y0 = g.pow(Scalar(3));
    let y1 = g.pow(Scalar(7));
    let sid = b"sigma-or-dsl-fischlin-bug";
    ...
    let _ = fsr_proof_dsl::prove! {
        transform = "fischlin",
        oracle = FischlinOracle::new(HashOracle::new(b"YavOracle/SigmaOR/DSL/Fischlin/Bug"), FischlinParams::new(rho, b_bits)),
        rho = rho, b = b_bits,
